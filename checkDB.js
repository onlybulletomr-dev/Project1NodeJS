...
async function checkDatabase() {
  try {
    ...
    const payments = await client.query(`
      SELECT paymentid, invoiceid, amount, paymentstatus, paymentdate 
      FROM paymentdetail 
    ...
    payments.rows.forEach(row => {
      ...
      console.log(`  Payment ${row.paymentid}: ${invText}, Amount=₹${row.amount}, Status=${row.paymentstatus}, Date=${row.paymentdate}`);
    });
